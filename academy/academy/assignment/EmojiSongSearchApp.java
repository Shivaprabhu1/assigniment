...
import org.academy.assignment.service.EmojiJdbcService;
...
public class EmojiSongSearchApp extends JFrame{
	...
	private void displaySongsInTable(List<Song> songs) {
		String[] columnNames = { "author", "movie_name", "music_producer", "name", "singer" };
		Object[][] data = new Object[songs.size()][6];
		for (int i = 0; i < songs.size(); i++) {
			//data[i][0] = songs.get(i).getId();
			data[i][0] = songs.get(i).getAuthor();
			data[i][1] = songs.get(i).getDescription();
			data[i][2] = songs.get(i).getFullLyrics();
			data[i][3] = songs.get(i).getName();
			data[i][4] = songs.get(i).getSinger();
		}
